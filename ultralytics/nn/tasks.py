...
def guess_model_task(model):
    ...
    def cfg2task(cfg):
        ...
        if m in {"classify", "classifier", "cls", "fc"}:
            return "classify"
        # todo 注册检测头魔改后任务为detect
        if m == "detect" or m in [item.__name__.lower() for item in add_detect]:
            return "detect"
        if m == "segment":
            return "segment"
        if m == "pose":
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Segment):
                ...
                return "obb"
            # todo 虽然也在这里添加,但似乎代码从没走到过这一步
            elif isinstance(m, (Detect, WorldDetect, *add_detect)):
                return "detect"
